    ...
    try:
        order = portfolio_service.create_order(
            ...
            OrderCreate(
                ...
                allow_fractional_resize=payload.allow_fractional_resize,
...
def get_automation(db: Session = Depends(get_db)) -> TradingAutomationProfileRead:
    stored_profile, source_profile, inherit_from_live = trading_workspace_service.resolve_profile_pair(db, "simulation")
...
def approve_signal(signal_id: str, payload: SignalApproveRequest | None = None, db: Session = Depends(get_db)) -> AutomationDecisionRead:
    try:
        result = trading_workspace_service.approve_signal(db, "simulation", signal_id, payload.reason if payload else None)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    return AutomationDecisionRead.model_validate(result)
